...
const authorIDByName = require('./authorIDByName');
...
const entities = fs.readdirSync('../../src/main/ml-modules/options').filter(file => {
  return file.match(/\.xml$/);
}).map(file => {
  return file.replace(/\.xml$/, '');
});

// Provide routes for each entity, and pseudo-type 'all'
const types = entities.concat([
  'all'
]);
types.forEach(type => {
  // Search route
