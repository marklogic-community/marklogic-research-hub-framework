...
function main(contentDesc, options) {
  ...
  // Do nothing if a Protein with the same Full Name is already existing
  //TODO: Need to categorize the Fullname to a family this case ns1 types
  let targetDb = xdmp.database(options['targetDatabase']);
  let evalParams = { "fullName" : content.fullName };
  let evalOptions = { "database": targetDb };
  let result = xdmp.eval(`
          let proteinDupFinderQuery = cts.andQuery([
          cts.collectionQuery(['Protein']),
          cts.jsonPropertyValueQuery('fullName', fullName)
  ]);
          let proteinDupFinderQueryOptions = [
          "limit=1"
  ];
          cts.uris("", proteinDupFinderQueryOptions, proteinDupFinderQuery);
