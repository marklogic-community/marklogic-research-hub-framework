...
module namespace tr = "http://marklogic.com/phr/lib/enrichment/text-rank";

import module namespace rf = "http://marklogic.com/phr/lib/enrichment/related" at "/lib/enrichment/related.xqy";
...
declare function tr:enrich( $data as element(), $options as map:map* ) {
    let $return-text as xs:boolean :=  fn:not(map:get($options, "returnText") = "false")
    let $iterations as xs:int := xs:int( (map:get($options, "iterations"), 50)[1] )
    let $threshold as xs:double := xs:double( (map:get($options, "threshold"), 0.05)[1]  )
    let $remove-stop as xs:boolean := fn:not(map:get($options, "removeStopWords") = "false")
    let $remove-noise as xs:boolean := fn:not(map:get($options, "removeNoiseWords") = "false")
    let $related-articles as xs:boolean := fn:not(map:get($options, "relatedArticles") = "false")
    let $lang := "eng"
    let $xml :=
...
declare function tr:find-best-terms( $word-map, $threshold as xs:double ) {
    let $terms :=
        <mle:terms>{
            ...
            where $score gt $threshold
            order by $score descending
            return <mle:term score="{ $score }" count="{ $count }">{ $word }</mle:term>
        }</mle:terms>
    let $total-terms := fn:count( $terms/mle:term )
    return $terms
};
...
declare function tr:match-concepts( $word-map, $terms, $lang ) {
  <mle:concepts>{
       tr:match-concepts-genes($word-map, $terms, $lang)
  }</mle:concepts>
};

(: Get top concepts :)
declare function tr:match-concepts-genes($wordMap, $terms, $lang ) {
  <mle:genes>{
        let $search := 
          cts:search(
            fn:doc(), 
            cts:and-query((
                cts:reverse-query( $terms ),
                cts:collection-query("Gene")
                ))
            )
        ...
        let $uri as xs:string* := xdmp:node-uri($enrichmentObjectNode)
        ...
        let $matchCount := fn:count( $matches )
