  ...
  return (
    <div className="col-md-9">
      <section id="top" className="card-list quick-section">
        ...
        <h2>
          <i className="fas fa-syringe" /> {study.title || id}
        </h2>
      </section>
      {study.title && (
        <>
          <section id="description" className="quick-section">
            ...
            <h6>Study Ids: {study.studyIds}</h6>
            <h6>Arm Count: {study.armCount}</h6>
            <a href={study.linkUrl} rel="noopener noreferrer" target="_blank">
              Clinical Trials Link
            </a>
            <pre>{study.summary}</pre>
          </section>
        </>
      )}
    </div>
...
class StudyDetailView extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    const study = this.props.detail && this.props.detail;
          ...
          <Col md={1}>
            <Bookmark
              entity={{
                ...
                type: 'study',
                label: study.title || this.props.id
              }}
            />
          </Col>
          <Col md={11}>
...
StudyDetailView.defaultProps = {
  template: ViewAsJson
};

StudyDetailView.propTypes = {
