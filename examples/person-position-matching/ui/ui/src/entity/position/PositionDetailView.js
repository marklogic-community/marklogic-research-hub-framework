  ...
  return (
    <div className="col-md-9">
      <section id="top" className="card-list quick-section">
        ...
        <h1>
          <i className="fas fa-briefcase" /> {position.description || id}
        </h1>
        <dl className="dl">
          <dt>Required Skills</dt>
          <dd>
            <ul>{
              position.requiredSkill.map(skill => <li key={skill}>{ skill }</li>)
            }</ul>
          </dd>
        </dl>
      </section>
...
class PositionDetailView extends Component {
  ...
  render() {
    const position = this.props.detail && this.props.detail;
          ...
          <Col md={1}>
            <Bookmark
              entity={{
                ...
                type: 'position',
                label: position.description || this.props.id
              }}
            />
          </Col>
          <Col md={11}>
...
PositionDetailView.defaultProps = {
  template: ViewAsJson
};
