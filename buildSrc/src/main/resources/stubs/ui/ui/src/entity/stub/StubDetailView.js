  ...
  return (
    <div className="col-md-9">
      <section id="top" className="card-list quick-section">
        ...
        <h1>
          <i className="fas fa-bullseye" /> {stub.name || id}
        </h1>
      </section>
    </div>
  );
...
class StubDetailView extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    const stub = this.props.detail && this.props.detail;
    if (!stub) return null;
    return (
      <>
        <DetailViewHeader
...
StubDetailView.defaultProps = {
  template: ViewAsJson
};

StubDetailView.propTypes = {
